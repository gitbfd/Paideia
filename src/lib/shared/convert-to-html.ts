// src/lib/shared/convert-to-html.ts
// Cross-stage HTML conversion utility
// Used in both INGEST (document-processor) and DISPLAY (preview reconstruction)

import { marked } from 'marked';
import { textProcessingConfig } from '../ingest/shared/config';
import { postProcessMarkdownHtml } from '../display/shared/markdown-post-processor';
...
/**
 * Converts markdown to HTML using standard GFM, then applies custom post-processing rules
 */
async function markdownToHtml(text: string): Promise<string> {
  ...
  try {
    ...
    // Apply custom post-processing rules (markdown-specific)
    html = postProcessMarkdownHtml(html);
