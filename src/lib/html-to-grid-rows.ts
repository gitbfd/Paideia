...
import { wrapHtmlContent } from './wrap-html';
 ...
 * 2. If text is longer than char limit, include a <br> tag
 * 3. Every <br> tag creates a new grid row
 * 4. DO NOT add <p> tags unnecessarily
...
export function htmlToGridRows(html: string, startLineNumber: number = 1): GridRow[] {
  ...
  // Step 1.5: Replace all remaining </p> tags with <br></p>
  // This ensures every paragraph's final line creates a new grid row
  // BUT: Don't add <br> if the paragraph is immediately followed by an empty <p></p>
  // First, mark empty paragraphs that come after </p> so we can handle them specially
  let htmlWithParagraphBreaks = cleanedHtml.replace(/<\/p>/gi, '<br></p>');
  
  // Remove any <br></p> that is immediately followed by an empty <p></p>
  // This prevents creating a <br> before an empty paragraph
  htmlWithParagraphBreaks = htmlWithParagraphBreaks.replace(/<br><\/p>\s*<p[^>]*>\s*<\/p>/gi, '</p>');
  
  // Step 2: Wrap HTML with <br> tags to impose width restrictions (for text longer than char limit)
  // This adds <br> tags where text exceeds the character limit
  const wrapped = wrapHtmlContent(htmlWithParagraphBreaks);
